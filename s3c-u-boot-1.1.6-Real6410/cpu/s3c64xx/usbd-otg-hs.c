/*
 * cpu/s3c64xx/usbd-otg-hs.c
 *
 *  $Id: usbd-otg-hs.c,v 1.3 2008/07/10 08:19:19 boyko Exp $
 *
 * (C) Copyright 2007
...
/* S3C_OTG_GINTSTS/S3C_OTG_GINTMSK core interrupt register */
...
/* S3C_OTG_DCTL device control register */
...
/* S3C_OTG_DAINT device all endpoint interrupt register */
...
/* S3C_OTG_DIEPCTL0/S3C_OTG_DOEPCTL0 */
...
unsigned UsbDownloadChecksumOK, UsbDownloadFileSize;
...
void s3c_usb_verify_checksum(void)
{
	...
	UsbDownloadFileSize = otg.dn_filesize-10;
	...
	UsbDownloadChecksumOK = checkSum == dnCS;
...
void s3c_usb_set_feature(void)
{
	...
	case TEST_MODE:
		...
		case TEST_PACKET:
			...
			writel(tmp, S3C_OTG_DCTL);
			DBG_SETUP1 ("S3C_OTG_DCTL=0x%08x\n", tmp);
			break;
		}
		break;
